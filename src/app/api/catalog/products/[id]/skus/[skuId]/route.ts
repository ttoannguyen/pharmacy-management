...
import { CatalogConflictError, CatalogNotFoundError, archiveStoreSkuSchema } from "@/modules/catalog/application/store-product-service";
import { archiveStoreSku } from "@/modules/catalog/infrastructure/prisma-store-product-service";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string; skuId: string }> }) {
  try {
    ...
    const parsed = archiveStoreSkuSchema.safeParse(await request.json().catch(() => ({})));
